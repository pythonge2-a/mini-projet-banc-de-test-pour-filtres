...
class MainFrame(wx.Frame):
    def __init__(self):
        ...
        freq_title = wx.StaticText(
            panel, label="Frequency Configuration:", pos=(20, 10)
        )
        ...
        points_title = wx.StaticText(
            panel, label="Number of Points Configuration:", pos=(20, 110)
        )
        ...
        self.unitPoint = wx.ComboBox(
            panel, pos=(300, 138), choices=["-", "k", "M", "G"], style=wx.CB_READONLY
        )
        self.unitPoint.SetSelection(0)

        # ---- Configuration de l'amplitude ----
        amp_title = wx.StaticText(
            panel, label="Amplitude Configuration:", pos=(20, 180)
        )
        ...
        connection_title = wx.StaticText(
            panel, label="Ethernet Configuration:", pos=(20, 250)
        )
        ...
        for device in self.devices:
            ...
            self.ip_controls[device] = wx.TextCtrl(
                panel, pos=(20, y_pos + 20), size=(260, -1)
            )
            y_pos += 60

        # Checkbox pour sélectionner le multimètre
        self.use_multimeter = wx.CheckBox(
            panel, label="Use Multimeter", pos=(20, y_pos)
        )
        self.use_multimeter.Bind(wx.EVT_CHECKBOX, self.on_multimeter_checkbox)

        # Champ IP pour le Multimètre (initialement caché)
        self.multimeter_ip_label = wx.StaticText(
            panel, label="Multimeter IP Address:", pos=(20, y_pos + 30)
        )
        self.multimeter_ip_ctrl = wx.TextCtrl(
            panel, pos=(20, y_pos + 50), size=(260, -1)
        )
        ...
        # Bouton : Start test
        button = wx.Button(
            panel, label="Start Test", pos=(130, y_pos + 90), size=(140, 40)
        )
    ...
    def get_ip_addresses(self):
        ...
        ip_addresses = {
            device: ctrl.GetValue() for device, ctrl in self.ip_controls.items()
        }
        if self.use_multimeter.IsChecked():
    ...
    def on_button_click(self, event):
        ...
        if (
            not self.min_freq_ctrl.GetValue().replace(".", "").isdigit()
            or not self.max_freq_ctrl.GetValue().replace(".", "").isdigit()
        ):
            wx.MessageBox(
                "Please enter only numbers for frequency.",
                "Error",
                wx.OK | wx.ICON_ERROR,
            )
            return
        ...
        if not self.points_ctrl.GetValue().replace(".", "").isdigit():
            wx.MessageBox(
                "Please enter only numbers for number of points.",
                "Error",
                wx.OK | wx.ICON_ERROR,
            )
            return
        ...
        if not self.amp_ctrl.GetValue().replace(".", "").isdigit():
            wx.MessageBox(
                "Please enter only numbers for amplitude.",
                "Error",
                wx.OK | wx.ICON_ERROR,
            )
            return
        ...
        for device, ctrl in self.ip_controls.items():
            if not ctrl.GetValue().replace(".", "").isdigit():
                wx.MessageBox(
                    "Please enter only numbers for IP addresses.",
                    "Error",
                    wx.OK | wx.ICON_ERROR,
                )
                return
        ...
        if not self.validate_inputs():
            wx.MessageBox(
                "Please specify all configuration values.",
                "Error",
                wx.OK | wx.ICON_ERROR,
            )
        else:
    ...
    def test_results(self):
        ...
        try:
            ...
            wx.MessageBox(
                f"Erreur lors de la connexion ou de l'acquisition : {e}",
                "Error",
                wx.OK | wx.ICON_ERROR,
            )
            return
